"""
Inference for the SwinUNETR dental segmentation model.

Matches train_swin.py: LetterboxResized preprocessing (no in-pipeline CLAHE),
...
fold_paths = [os.path.join(SAVE_ROOT, f"swin_fold{f}.pth") for f in range(NUM_FOLDS)]
fold_paths = [p for p in fold_paths if os.path.exists(p)]
if not fold_paths:
    raise FileNotFoundError(
        f"No fold checkpoints (swin_fold0.pth ..) in {SAVE_ROOT}. Run train_swin.py first."
    )
