"""
SwinUNETR segmentation for dental X-ray multi-class segmentation.
...
Ported to match train_unet.py's design (2026-05-26):
  ...
  * LetterboxResized scales the longest edge to a square canvas and pads the
    ...
    plain crop of [:nA, :nB] then resize (see test_swin.py).
  * Leakage guard: an image and its augmentations never straddle the split.

Inherited fixes:
  1. 3-channel input (RepeatChanneld) preserves ImageNet-style encoder capacity.
...
NUM_CLASSES = 12
CANVAS = 1024          # square letterbox canvas; longest image edge maps here (1024/32=32, valid for SwinUNETR)
NUM_FOLDS = 5
BATCH_SIZE = 4         # SwinUNETR is memory-heavy at 1024²; raise to 6-8 if your GPU allows
MAX_EPOCHS = 300
WARMUP_EPOCHS = 5
...
def train_one_fold(fold, train_dicts, val_dicts):
    ...
    best_path = os.path.join(SAVE_ROOT, f"swin_fold{fold}.pth")
    log_path = os.path.join(SAVE_ROOT, f"swin_fold{fold}_val_log.csv")
...
def main():
    ...
    print("Fold checkpoints: swin_fold0.pth .. swin_fold{}.pth".format(NUM_FOLDS - 1))
