"""
Inference for the EfficientNet-B2 U-Net dental segmentation model.

Matches train_unet.py: LetterboxResized preprocessing (no in-pipeline CLAHE),
...
output_dir      = "/home/jiakuny1/Projects/unet_predictions"
...
fold_paths = [os.path.join(SAVE_ROOT, f"unet_fold{f}.pth") for f in range(NUM_FOLDS)]
fold_paths = [p for p in fold_paths if os.path.exists(p)]
if not fold_paths:
    raise FileNotFoundError(
        f"No fold checkpoints (unet_fold0.pth ..) in {SAVE_ROOT}. Run train_unet.py first."
    )
