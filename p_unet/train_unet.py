"""
U-Net segmentation for dental X-ray multi-class segmentation.

K-FOLD CROSS-VALIDATION + ASPECT-RATIO-PRESERVING (LETTERBOX) RESIZE.

Why this version exists (2026-05-22):
  * The previous version used a single 80/20 split: 88 of 436 images never
    contributed to training. With a small dataset that is wasteful, and the
    held-out estimate is noisy. K-fold CV trains on every image (across folds)
    and the K fold-models are ensembled at inference (nnU-Net's recipe).
  * The previous version forced every image to (896, 1024), which distorts the
    portrait scans (1200x852) and squishes the thin canal structures — exactly
    the weak classes. LetterboxResized scales the longest edge to a square
    canvas and pads the remainder, so geometry is preserved. Content sits at the
    top-left (origin); padding is added on the bottom/right, which makes the
    inverse at inference trivial and exact (see test_unet.py).

  NOTE: filenames carry no patient ID (433/436 share the "IO000001" prefix), so
  the folds are NOT patient-grouped. If two scans of the same patient exist they
  can land on opposite sides of a fold and inflate val. If you have patient IDs
  elsewhere, replace KFold with GroupKFold(groups=patient_ids).

Inherited fixes (still in place):
  ...
  4. Tversky(0.3/0.7)+Focal loss; copy-paste of rare-class crops.
...
NUM_FOLDS = 5
BATCH_SIZE = 4         # 1024x1024 uses ~30% more memory than the old 896x1024; raise if your GPU allows
MAX_EPOCHS = 300
WARMUP_EPOCHS = 5
...
class LetterboxResized(MapTransform):
    ...
    inverse at inference a plain crop of [:nA, :nB] followed by a resize — no
    centering arithmetic to get wrong (this is what test_unet.py relies on).
...
def train_one_fold(fold, train_dicts, val_dicts):
    ...
    best_path = os.path.join(SAVE_ROOT, f"unet_fold{fold}.pth")
    log_path = os.path.join(SAVE_ROOT, f"unet_fold{fold}_val_log.csv")
...
def main():
    ...
    print("Fold checkpoints: unet_fold0.pth .. unet_fold{}.pth".format(NUM_FOLDS - 1))
